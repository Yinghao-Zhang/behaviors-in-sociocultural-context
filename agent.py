...
class Agent:
    ...
    def add_setup(self, setup, affinity=None):
        """
        Add a new setup to the setups memory or update an existing setup's affinity.

        Parameters:
        - setup (Setup): a setup object.
        - affinity (float, optional): Setup affinity (unused, reserved for future use).
        """
    ...
    def _decision_weights(self, params):
        """Return intrapersonal decision weights, optionally normalized to a simplex."""
        w_I = getattr(self, 'w_instinct', 1.0)
        w_E = params.get('w_enjoyment', 0.5)
        w_U = params.get('w_utility', 0.5)
        if getattr(self, 'decision_weight_mode', 'raw') == 'relative':
            total = max(1e-8, w_I + w_E + w_U)
            return w_I / total, w_E / total, w_U / total
        return w_I, w_E, w_U

    def choice_value_from_components(self, instinct, enjoyment, utility, params=None, include_noise=True):
        """Compute the intrapersonal choice value from I/E/U components."""
        if params is None:
            params = {}
        w_I, w_E, w_U = self._decision_weights(params)
        noise_scale = getattr(self, 'noise_scale', 0.2)
        noise = np.random.normal(0, noise_scale) if include_noise else 0.0
        return w_I * instinct + w_E * enjoyment + w_U * utility + noise

    def form_intention(self, setup, behaviors=None):
        """
        Compute raw choice values for each behavior in a setup using the tripartite model.

        Probability conversion happens in select_behavior(). Keeping this method on
        the value scale lets social suggestion and co-participation blend like with like.

        Returns: dict mapping behavior to raw choice value
        """
        ...
        for behavior in filtered:
            ...
            choice_values[behavior] = self.choice_value_from_components(
                params['instinct'],
                params['enjoyment'],
                params['utility'],
                params=params,
    ...
    def select_behavior(self, setup, behaviors=None, choice_probs=None, return_probs=False):
        """
        Select a behavior using the agent's raw choice values.

        Parameters:
        - setup (Setup): The setup context used for intention formation.
        - behaviors (list[Behavior]): Optional list of behaviors to consider.
        - choice_probs (dict): Optional mapping of behaviors to raw selection values.
        - return_probs (bool): If True, return normalized probabilities with the selection.

        Returns:
        - Behavior: The selected behavior.
        - dict: Optional mapping of behaviors to normalized probabilities.
        """
        caller_supplied_values = choice_probs is not None
        if choice_probs is None:
            choice_probs = self.form_intention(setup, behaviors)

        if behaviors is not None:
            behaviors_list = [b for b in behaviors if b in choice_probs]
        else:
            behaviors_list = list(choice_probs.keys())

        if not behaviors_list:
            raise ValueError("No behaviors available for selection.")

        decision_model = getattr(self, 'decision_model', 'softmax')
        if decision_model == 'ddm' and len(behaviors_list) == 2:
            b0, b1 = sorted(behaviors_list, key=lambda b: b.name)
            if caller_supplied_values:
                # Social situations pass already-blended raw choice values. Treat
                # their difference as the drift signal while using a neutral
                # starting point, so the supplemental threshold generator is
                # still nonlinear without inventing extra social-bias terms.
                values = np.array([choice_probs[b0], choice_probs[b1]], dtype=float)
                values = np.where(np.isfinite(values), values, 0.0)
                drift = float(values[1] - values[0])
                z = 0.5
            else:
                params0 = self.behaviors[b0][setup]
                params1 = self.behaviors[b1][setup]

                w_I0, w_E0, w_U0 = self._decision_weights(params0)
                w_I1, w_E1, w_U1 = self._decision_weights(params1)
                eval0 = params0['enjoyment'] * w_E0 + params0['utility'] * w_U0
                eval1 = params1['enjoyment'] * w_E1 + params1['utility'] * w_U1
                bias_scale = np.mean([
                    params0.get('bias_scaling_factor', 1.0),
                    params1.get('bias_scaling_factor', 1.0),
                ])
                drift = eval1 - eval0
                bias = w_I1 * params1['instinct'] - w_I0 * params0['instinct']
                z = 1.0 / (1.0 + np.exp(-bias_scale * bias))
            boundary = getattr(self, 'ddm_boundary', 1.0)
            noise = getattr(self, 'ddm_noise', 1.0)

            if abs(drift) < 1e-8:
                p_upper = z
            else:
                scale = 2.0 * drift * boundary / (noise ** 2)
                denom = 1.0 - np.exp(-scale)
                p_upper = (1.0 - np.exp(-scale * z)) / denom

            p_upper = float(np.clip(p_upper, 1e-6, 1.0 - 1e-6))
            behaviors_list = [b0, b1]
            probs = np.array([1.0 - p_upper, p_upper], dtype=float)
            selected_idx = np.random.choice(len(behaviors_list), p=probs)
            selected = behaviors_list[selected_idx]
            if return_probs:
                return selected, {b: float(p) for b, p in zip(behaviors_list, probs)}
            return selected

        values = np.array([choice_probs[b] for b in behaviors_list], dtype=float)
        values = np.where(np.isfinite(values), values, 0.0)
        tau = getattr(self, 'tau', 2.0)
        scaled = tau * (values - np.max(values))
        exp_vals = np.exp(scaled)
        total = np.sum(exp_vals)
        if total <= 0 or not np.isfinite(total):
            probs = np.ones(len(behaviors_list)) / len(behaviors_list)
        else:
